  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3003/leagueoflegends/accounts?${queryParams.toString()}`)
        ...
        if (response.data?.data) {
          ...
          // Use the seller stats directly from API response
          if (response.data.data.sellerStats) {
            setSellerStats(response.data.data.sellerStats)
          }
          
          // Create a dictionary of sellers from populated accounts
          const sellerMap = {}
          response.data.data.accounts.forEach(account => {
            if (account.sellerID) {
              sellerMap[account.sellerID._id] = account.sellerID
            }
          })
              ...
              >
                ...
                <div className="p-3 border-t border-gray-800 flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-1 text-green-500">
                    <span>{sellerStats[account.sellerID?._id]?.rating || 98}%</span>
